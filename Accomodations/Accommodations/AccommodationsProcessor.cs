...
public static class AccommodationsProcessor
{
    ...
    public static int? ParserId(string value)
    {
        try
        {
    ...
    private static void ProcessCommand(string input)
    {
        ...
        switch (commandName)
        {
            case "book":
                ...
                //вызываем метод Parse для нач и конеч даты
                int? idParse = ParserId(parts[1]);
                idParse.ThrowIfNull("ID");
                DateTime? beginParse = ParserDate(parts[3]);
                DateTime? endParse = ParserDate(parts[4]);

                //добавил обработку исключений для некорректного курса
                ...
                BookingDto bookingDto = new()
                {
                    UserId = (int)idParse,
                    Category = parts[2],
                    StartDate = (DateTime)beginParse,
                    EndDate = (DateTime)endParse,
                    Currency = currency,
                };
